...
En Codex los bins **no están en el PATH** — se invocan por ruta completa, con `sh "<RAIZ>/core/bin/<bin>"`.
...
- Un aviso de cierre de turno dice que hay trabajo en ESTA sesión sin registrar.
...
- pasos intermedios de algo que todavía no cerró — para eso está la skill `one-brain:handoff`;
...
No entra acá: buenas prácticas de cómo trabajar con el asistente (mezclan dos audiencias en la
misma búsqueda), lo que ya está escrito en el repo o en un README (la memoria apunta, no copia),
ni lo específico de una sesión que no se va a repetir.
...
`sh "<RAIZ>/core/bin/onebrain-save" --type procedimiento --title "Despausar el Supabase de Lempriere" --content "1. …" --entities "Lempriere"`
...
0. **Chequeá el feature.** Corré: `sh "<RAIZ>/core/bin/onebrain-feature" auto-capture`. Si sale
   con exit 1 (el usuario desactivó "Captura automática" al cerrar sesión), NO hagas nada:
   terminá en silencio.
...
4. Con el OK → guardá cada entry por el **canal Bash**, que no depende de que la tool MCP esté
   cargada en esta sesión:
       ...
       sh "<RAIZ>/core/bin/onebrain-save" --type <type> --title "<title>" \
          --content "<content_md>" --entities "a,b"
   ...
   Si la tool `brain_save` del server MCP `one-brain` está disponible en la sesión, también sirve
   (mismo destino). El canal Bash es el que conviene por default porque anda siempre.
5. Si el usuario descarta, no guardes. Si hubo varios frentes, varios entries. Si no hubo nada guardable, decilo y no inventes.
6. **Si estabas actuando sobre un rescate** (el aviso de arranque que menciona un transcript de
   una sesión anterior sin guardar): el aviso trae la RUTA del transcript, no el session-id, y en
   Codex el nombre del archivo **no es** el session-id — los rollouts se llaman
   `rollout-<fecha>-<uuid>.jsonl`. El session-id quedó anotado en la marca, así que sacalo de ahí:

       grep -l 'transcript=<la-ruta-del-aviso>' ~/.config/one-brain/pending/pending-*

   El session-id es el nombre de ese archivo sin el prefijo `pending-`. (Si el `grep` no devuelve
   nada, el session-id es el UUID del final del nombre del rollout: lo que sigue a la fecha, sin
   la extensión `.jsonl`.) Una vez que el guardado quedó CONFIRMADO (entry_id devuelto), corré
   `sh "<RAIZ>/core/bin/onebrain-resolve-pending" <session-id>` para borrar esa marca. Si el
   guardado FALLÓ o quedó encolado, **NO** la borres: tiene que seguir avisando hasta que se
   guarde de verdad. El aviso insiste hasta 3 arranques y después descarta la marca solo (la
   última vez lo dice explícitamente): si ves esa advertencia, es la última oportunidad de
   rescatar ese transcript — decíselo al usuario antes de seguir con otra cosa.

## Reglas
- Nunca guardes datos personales sensibles ni secrets.
- Si el guardado falla (server pausado, red), avisá al usuario y NO des el avance por perdido: quedará pendiente para el próximo cierre.
